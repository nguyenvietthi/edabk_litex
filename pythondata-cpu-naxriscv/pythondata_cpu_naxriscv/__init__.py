...
src = "https://github.com/SpinalHDL/NaxRiscv.git"
...
def data_file(f):
    """Get absolute path for file inside pythondata_cpu_naxriscv."""
    fn = os.path.join(data_location, f)
    fn = os.path.abspath(fn)
    if not os.path.exists(fn):
        raise IOError("File {f} doesn't exist in pythondata_cpu_naxriscv".format(f))
    return fn
